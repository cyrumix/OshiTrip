...
- **共有概算経路（`shared_route_estimates`）のFlutter側再利用は未実装**（修正4）。
  Phase 4ではDB基盤（スキーマ・RLS・モデレーション・不変条件・pgTAP）までとし、
  owner境界・approvedのみ・data_origin/rights_basis確認つきのクライアント検索・
  通常表示は [follow-up-work.md 項目21](follow-up-work.md) へ切り出した。旅程内の
  「保存済み概算を優先」は各旅程の `itinerary_legs` で機能的に満たしている。
