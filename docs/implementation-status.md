...
## 旅程Phase 4（Google Routes連携）— 状態: 🟡 コード実装済み・環境依存の検証と一部再利用UIが未完

実装判断・確認記録は [decisions.md「旅程Phase 4」節・「旅程Phase 4 レビュー是正」節](decisions.md) を正とする。

### 実装済み（オフライン検証可能）

- RoutesGateway 実装（`RoutesGatewayImpl` / `SupabaseRoutesProxyTransport`）を
  `routesGatewayProvider` へ接続。有効環境（Routes有効＋Supabaseクライアントあり）で
  routes-proxy Edge Function を呼び、無効・デモ・未設定では `UnavailableRoutesGateway`
  へ縮退する（修正1）。Google API キーはアプリに埋め込まない（routes-proxy が保持）。
- entitlement 取得に共通タイムアウト（`.withRemoteTimeout()`）と認証切替 `isStale`
  チェックを追加（修正2・修正3）。TimeoutException は NetworkFailure。
- 公共交通の代表時刻が対応範囲（過去7日〜未来100日）外なら、Google Routes を
  呼ぶ前に UI 案内を出す（修正5）。保存済み概算・手動入力は引き続き利用可能。
- 費用制御（明示操作のみ・single-flight・Field Mask allowlist=Essentials維持）、
  Googleライブ結果を `ItineraryLeg`／共有DBへ永続保存しない方針は維持。
- 共有概算経路（`shared_route_estimates`）の**再利用強制ゲートを純粋ドメイン層として
  先出し**（`shared_route_estimate.dart`: `parseSharedRouteEstimate` /
  `sharedRouteEstimateReuseError`）。再利用の読み取りは必ずこのゲートを通し、
  approved のみ・data_origin4種（'google'等は型で不能）・rights_basis 非空を満たさない
  行を採用しない（D-224、shared_facility.dart と同じ先出し方針）。
- entitlement 取得を `EntitlementFetcher` seam へ切り出し、timeout/成功/失敗/isStale の
  全経路を単体テスト化（D-225）。

### 未実装（明示）

- **共有概算経路（`shared_route_estimates`）のUI再利用・読み取りRepositoryは未実装**。
  強制ゲート（上記）は先出し済みだが、UI表示（旅程スポット→施設ID解決→パネル表示）は
  施設ID解決に必要な shared_facilities の Flutter クライアント（D-209で次Phase送り・
  現状 lib/ に一切存在しない）が前提のため、[follow-up-work.md 項目21](follow-up-work.md)
  へ切り出した。旅程内の「保存済み概算を優先／Google自動呼び出しなし」は各旅程の
  `itinerary_legs` と route_live_panel（明示タップのみ）で機能的に満たしている。

### 未検証（環境依存。成功扱いにしない）

- 実 Google Routes 呼び出し・routes-proxy 実デプロイ（Docker／Deno／Google 鍵なし）。
- `supabase test db`（pgTAP `0014`）— Docker 未導入で未実行（静的レビューのみ）。
- entitlement 付与の実運用（service_role からの INSERT/UPDATE）・実機E2E。

### 旅程MVP完成判定

**未完成**。requirements.md §7.9 の「Phase 4 完了まで旅程MVP完成と判定しない」に対し、
(1) 共有概算経路のFlutter側再利用が未実装、(2) 実 Google Routes 呼び出し・pgTAP・
実機検証が環境不足で未了、の2点により、旅程MVPは完成扱いにしない。

## 旅程Phase 5（共有・共同編集・通知・最終品質）— 状態: 🟡 前提「共有データ基盤」のみ実装

実装判断は [decisions.md「旅程Phase 5 依存ギャップ＋共有データ基盤」節](decisions.md)（D-226/D-227）を正とする。

### 依存ギャップの結論

Phase 5本体（共有・共同編集・通知）の前提基盤である**現場共有・通知基盤はドメイン型のみの
スタブで未実装**だった（genba_shares 表・ShareRepository実装・ロールRLS・NotificationScheduler
実装・配線が皆無）。「前提基盤が未完成なら見せかけUIを作らず依存不足を報告し先に基盤を作る」に
従い、Phase 5本体には着手せず**共有データ基盤（保守的スライス）**を実装した。

### 実装済み（オフライン検証可能）

- `genba_shares` 共有データ基盤: Supabase `0028`（表＋**owner 管理 RLS**＋grantee 自共有 SELECT のみ＋
  子owner トリガ＋CHECK＋apply_mutation 登録）、Drift v17（新規追加のみ・既存無改変）、
  ShareRepository 実装（owner スコープ CRUD＋親現場所有権検証＋Outbox＋同期）、ドメイン
  （role/field_grants/不変条件）。**既存データ表の RLS は一切変更していない**（C-01 リスクゼロ）。
- Dart テスト: 共有 CRUD・owner 分離・親owner整合・owner偽装・自己共有拒否・v16→v17移行・不変条件。

### 未実装（明示・次増分）

- grantee による共有データの**実 read/write のロール別 RLS**、**項目マスキング（view/カラム）**、
  **Storage 共有ポリシー**、**editor write-through**（apply_mutation の owner矯正＋子ownerトリガ＋
  書込RLS改修）、**Realtime 共同編集**、**共有向け競合UI**、**共有解除後のキャッシュ到達不能の
  クライアント処理**、**招待/項目共有設定 UI**。→ pgTAP を実行検証できる環境（Docker/CI）で行う。
- **通知基盤**（NotificationScheduler 実装・FCM/APNs・許可UX・ディープリンク・冪等/同日集約）。
  → device/Firebase 依存で別増分（D-227）。
- **思い出連携・最終品質収束・運用監視・プレミアム機能フラグ境界**（Phase 5 項目5〜8）。

### 未検証（環境依存。成功扱いにしない）

- `supabase test db`（pgTAP `0015_genba_shares.sql`）— Docker/Supabase CLI 未導入で未実行（静的レビューのみ）。
- ロール別 read/write RLS・項目マスキング・Storage 共有の実挙動、iOS 実機/VoiceOver/通知 E2E。

### Phase 5 完成判定

**未完成**。共有**データ基盤**のみ実装。共有の実 read/write・項目マスキング・Storage・共同編集・
競合UI・通知・実機/pgTAP 検証は次増分。AI旅程・ルート最適化・本課金は引き続きバックログ。
