...
8. 通常表示は権利確認済みの保存済み概算経路を優先する。Google Routesはユーザーが「経路を確認」か「最新の経路」を押した場合だけ呼び、編集・並び替え・初期表示では呼ばない。**【現在はD-232で上書き】** かつては「プレミアムユーザーだけ」呼べる方針だったが、現仕様では全認証ユーザーが利用可能。プレミアム制限は Edge Function `ROUTES_REQUIRE_PREMIUM=true`（既定 false）の環境でのみ有効化する（将来のプレミアム化用に entitlement 機構は温存）。
...
- 全ユーザーが保存済み概算経路を閲覧できることをテストする。**【D-232】** 現仕様では経路のライブ取得も全認証ユーザーに開放（クライアント側のプレミアム拒否は無し）。サーバー側の entitlement 検証は `ROUTES_REQUIRE_PREMIUM=true`（既定 false）の環境でのみ発生し、その分岐は純粋関数 `premiumGateError`／`isFlagOn` で検証する
