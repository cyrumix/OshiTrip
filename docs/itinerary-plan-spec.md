...
- Google呼び出しはプレミアム権限をサーバーで検証し、非プレミアムには保存済み概算経路と手動編集を提供する
...
- 未計算区間でも自動取得せず、プレミアムユーザーが詳細を開くか`最新ルートを更新`を押した場合だけ取得する
...
| 基本旅程・日別タイムライン | Google Routes最新経路の明示更新 |
...
非プレミアムも保存済みの権利確認済み概算経路を閲覧できる。Google Routesのライブ取得・`最新ルートを更新`はプレミアム候補とし、サーバー側entitlementで強制する。旅程作成時に全区間を自動取得する機能は無料・プレミアムとも提供しない。
