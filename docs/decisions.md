...
| ID | 決定 | 理由 |
|----|------|------|
| D-252 | 思い出機能を「半券コレクション」へ全面再設計（design-spec §8/§9 改訂）。本質＝**終わった現場をそのまま残して見返す**の実装が、現状は「後入力した感想/セトリ/グッズだけを別フォームで見せる」ものになっており、現場そのもの（会場・チケット・座席・交通・宿泊・スポット）が思い出に現れない＝本質の不理解、と診断。方針: (a) 思い出=現場の凍結ビュー（現場に登録済みデータを自動で再掲。ユーザー再入力は当日しか書けない物＝感想/写真/セトリのみ）、(b) 詳細はタブ廃止・縦1本のストーリー（空セクション非表示・§9）、(c) 一覧は年グルーピング＋「n現場・n参戦」サマリーの半券ホルダー・灰色プレースホルダー全廃（§8）、(d) 記録UXは巨大1枚フォームをセクション別ボトムシートへ分解・写真は複数選択、(e) 終演直後の「おつかれさま」3ステップ導線。**スキーマ変更なし**（MemoryEntry/Photo/Setlist/Goods/VisitedPlace・同期・共有・写真状態(§12)は不変。年サマリー・遠征ダイジェストは既存データから導出）。段階実装 M1:一覧刷新 / M2:詳細ストーリー化＋現場データ自動引き継ぎ / M3:記録UX分解 / M4:もぎり導線 / M5(任意):年間まとめ | 現場一覧を「チケット」化した流れ（D-250/251）を「半券」で完結させ、UI以前の情報設計の誤りを正す。同一genba ID・孤立レコード非作成・attendance の意味論・loading/error 伝播（§15）は維持し、リスクの高い write-path やスキーマには触れない |

### 実装・テスト（D-252 / M1: 一覧刷新）

- `memory_list_screen.dart`: フィルタ適用後の思い出を**年ごとにグルーピング**（新しい年順）。各年に `_YearHeader`（「n現場・n参戦」サマリー）＋半券カード群。フィルタ（すべて/参戦済み/お気に入り）・お気に入りの全bundle購読・loading/error 伝播・中止の非消失・空状態は不変。
- `photo_memory_card.dart`: 2形態へ。表紙写真あり=写真ヒーロー＋ミシン目＋メタ帯（半券）／表紙写真なし=**券面デザイン（灰色プレースホルダー全廃）**。表紙が「あるはずが表示不可」の場合は理由を明示（§12）。`find.byType(PhotoMemoryCard)`・参戦済みバッジ・`FavoriteButton` は維持。
### 実装・テスト（D-252 / M2: 詳細のストーリー化＋現場データ自動引き継ぎ）

- `memory_detail_screen.dart`: **タブ（感想/セトリ/グッズ/メモ）を廃止**し、縦1本のストーリーへ（`_Story`）。セクションは 感想 → セトリ → その日の記録 → 遠征の記録 → タグ の順で、**空セクションは非表示**。末尾に「追記できること」チップ（`〜を追加`）。
- **現場データの自動引き継ぎ（本質の中核）**: `その日の記録` はチケットの座席（`Ticket.seat`）を自動再掲（`link` アイコン＋Semantics「現場から自動引き継ぎ」）＋見え方/MC/実際の終演時間。`遠征の記録` は現場の交通（`aggregate.transports`）・宿泊（`aggregate.lodgings`）を読み取り専用ダイジェストで再掲（「現場から自動」バッジ）＋思い出側の行った場所/食べたもの/グッズ。全記録・全引き継ぎが空のときのみ「まだ記録がありません」。
- お気に入り・写真カルーセル(1/N)・画像状態(§12)・アルバム入口・FAB「記録する」は維持（セクション別編集シートは M3）。
- テスト（`memory_detail_view_test.dart`）: タブ廃止（`SegmentTabs` findsNothing）＋感想/セトリ同時表示、写真なし縮退、記録ゼロ縮退、**現場の交通・宿泊・座席が自動引き継ぎされ空セクションは出ない**を追加。全 1024 テスト緑。

### 実装・テスト（D-252 / M3: 記録UXのセクション別シート分解＋複数写真選択）

- 巨大な1枚フォームを解体。共有編集ウィジェットを `memory_edit_widgets.dart` へ抽出（`ListEditor`/`TagEditor`/`ItemsWithPhotosEditor`/`PhotoThumb`/`ActualEndTimeCard`）し、フル記録画面（`MemoryEditScreen`）とシートで再利用（二重化回避）。
- `memory_edit_sheets.dart`: `showMemorySectionSheet`（感想／セトリ／座席・メモ／タグ／写真の**セクション別ボトムシート**・自動保存）＋ `showMemoryRecordMenu`（FAB「記録する」の入口メニュー）。込み入った物品編集（グッズ/場所/食べもの＝写真紐づけ・削除選択ダイアログ）はフル記録画面へ委譲。
- **複数写真選択**: `ImagePicker().pickImage`→`pickMultiImage`（フル画面・写真シートの双方。1枚ずつの制約を解消）。
- 詳細（その日のページ）: 各 `_MemorySection` に「編集」→当該シート、追記チップ→当該シート（遠征のみフル画面）、FAB→記録メニュー。
- テスト: 感想セクションの「編集」でそのセクションだけのシートが開き（他欄は無い）自動保存される／`main_flow` の記録導線を FAB→メニュー→感想シート→自動保存へ更新。既存の段階フォーム（`memory_edit_stages_test`）は抽出後も無改変で緑。全 1025 テスト緑。

### 実装・テスト（D-252 / M4: もぎり導線＝終演直後の3ステップ）

- `memory_afterparty_sheet.dart`: `showAfterEventSheet` を新設。終演直後（余韻中）に**①参戦した？（attended/notAttended を設定）→ ②写真を選ぶ（複数, `pickMultiImage`）→ ③ひとこと感想（自動保存）**の3ステップ。各ステップ**スキップ可**、完了で半券がコレクションに加わる。
- HOMEの `_AfterglowCard`（today_card）: CTA を「短い感想を書く」→**「思い出をまとめる」**へ変更し、フル記録画面ではなくもぎりシートを開く。参戦・写真・感想をその場で軽く残せる。
- テスト（`main_flow_test`）: 余韻中カード→もぎり→①参戦した→②写真スキップ→③ひとこと入力→完了で、**genba.attendanceStatus=attended かつ MemoryEntry.impression が保存**されることを検証。全 1025 テスト緑。

### 実装・テスト（D-252 / M5: 年間まとめ＝ふりかえり）

- `year_summary.dart`: `computeYearSummary(year, aggregates)` を**純粋関数**で新設（YearSummary＝現場数／参戦数／訪れた会場数／遠征した現場数／写真数／よく会いに行った推し・よく行った会場の最頻値）。既存の現場アグリゲートからの導出のみ・スキーマ変更なし。**「遠征距離」は経路データを保持していないため対象外**とし、代わりに遠征した現場数・訪れた会場数を集計（正直な代替）。最頻値の同数は先出現を優先（決定的）。
- `year_summary_sheet.dart`: 「◯年のふりかえり」シート（主役の大きな数字＝現場数・参戦数、よく会いに行った推し／よく行った会場のハイライト、訪れた会場・遠征・写真のミニスタッツ）。
- `memory_list_screen.dart`: 年ヘッダを**タップ可能**にし（Semantics「…。ふりかえりを見る」）ふりかえりシートを開く。
- テスト: `year_summary_test.dart`（純粋関数の集計・最頻値タイブレーク・空/未設定の頑健性）＋一覧の年ヘッダタップでシートが開き集計が出る。全 1029 テスト緑。

**D-252（思い出「半券コレクション」再設計）は M1〜M5 すべて完了**（本質是正: 終わった現場をそのまま残して見返す）。スキーマ・同期・共有・写真状態(§12)・attendance の意味論は全期間を通じて不変。

### レビュー是正（D-252 / codex レビュー指摘の修正）

Codex レビューの10指摘を修正（スキーマ変更なし・attendance/引き継ぎ読み取り専用/loading-error非潰しを維持）:

1. **もぎり attendance 取りこぼし**（`memory_afterparty_sheet.dart`）: `genbaByIdProvider` の loading/error/null を明示的に扱い、その場合は進めずメッセージ表示。`setAttendanceStatus` **成功時のみ** `_next()`（失敗時は留まる）。attended はユーザー明示操作の成功時のみ設定。
2. **pop 後 context 参照**（同）: `_finish` で pop 前に `ScaffoldMessengerState` を取得してから pop→表示。
3. **ふりかえり写真数が常に0**（`year_summary_sheet.dart` を `ConsumerWidget` 化・`memory_list_screen.dart`）: 対象年の各 genba の `memoryBundleProvider` を watch し確定分から `photoCounts` を集計して `computeYearSummary` に渡す。**loading=「集計中…」/error=「取得できませんでした」/確定=N枚** と区別（0に潰さない）。写真反映のテスト追加。
4. **写真だけの思い出が「まだ記録がありません」**（`memory_detail_screen.dart`）: `anyContent` に `bundle.photos.isNotEmpty` を含める。写真のみの思い出のテスト追加。
5. **pickMultiImage 例外未処理**（`memory_edit_widgets.dart` に共通 `pickPhotoPaths()` を新設、3画面で使用）: `PlatformException`/`Exception` を捕捉し「写真を選択できませんでした」を表示。キャンセル/選択/失敗を区別。
6. **ListEditor/ItemsWithPhotosEditor の追加が await されない**（`memory_edit_widgets.dart`）: 入力行を StatefulWidget `_AddRow` 化。`onAdd` を await し、**成功時のみ clear**（失敗時は入力を残しSnackBar）、処理中はボタン無効化で二重送信防止。呼び出し側の `onAdd` は `.then(_showFailure)` を外して失敗を返す契約に統一。
7. **ActualEndTimeCard の loading/error 非表示**（同）: `AsyncValue.when` で loading（読込中表示）/error（再試行）/データなし（見つからない表示）を区別。
8. **シート連続遷移の context 安全性**（`memory_edit_sheets.dart`）: メニュー pop 後の次シート/`context.push` を `context.mounted` で保護。
9. **追記チップのラベル依存遷移**（`memory_detail_screen.dart`）: 表示ラベルと遷移先を分離（`({String label, MemorySection? section})` モデル。`section==null`=遠征＝フル画面）。文言変更・多言語化で遷移先が変わらない。
10. **詳細ストーリーの一括ビルド**（同）: `_Story`（単一 `SliverToBoxAdapter`）を `_buildStorySlivers`（セクション毎の sliver 列）へ分割し、画面外セクションを遅延ビルド/カリング可能に。**残課題**: セクション内の多件数リスト（セトリ/遠征ダイジェスト）は現状 `Column` で一括ビルド。極端な件数では `SliverList` 化が望ましいが、通常運用では問題なく、今回の安全修正を優先（次増分）。

検証: `dart format`/`dart analyze`（クリーン）・`flutter test` **1031件 緑**。追加テスト: 写真だけの思い出は空状態にならない／ふりかえり写真数がバンドルから集計される。**未実施（実機依存・成功扱いにしない）**: 画像ピッカーの実機権限挙動・シートのキーボード時レイアウトは widget テスト対象外。

### レビュー是正 その2（非同期の堅牢性・残課題明記）

1. **写真ピッカー復帰後の mounted ガード**（`memory_afterparty_sheet.dart`/`memory_edit_sheets.dart`/`memory_edit_screen.dart`）: `pickPhotoPaths()`・`pickImage()` の await 直後に `if (!mounted) return;` を追加。複数追加の for ループでも各 `addPhoto` await の前後で mounted を確認し、**破棄済み State から `_controller`（`ref.read`）/`_showMessage`/`_showFailure` を呼ばない**。キャンセル・失敗・途中で閉じた場合を成功扱いしない。
2. **`_AddRow._submit` の例外時 busy 固着**（`memory_edit_widgets.dart`）: `try/catch(on Object)/finally` 構造へ。成功時のみ `clear()`、`Failure` 戻り・例外はいずれも入力を残して SnackBar 表示、**`finally` で必ず `busy=false`**（成功/失敗/例外/破棄すべてで復帰）。二重送信防止は維持。widget テスト追加（例外 throw で入力が残りボタン復帰／`Failure` 戻りでも残る／成功でクリア）。
3. **ふりかえりの大量 watch は残課題として記録**（`year_summary_sheet.dart`）: 対象年の全 `memoryBundleProvider` を watch する現行方式は、正しさ（loading/error を潰さない）は満たすが、1年に多数の現場があると重い可能性。ただし（a）シートを開いた時のみ購読、（b）1年あたりの件数は通常限定的、のため実害は小さい。**スキーマ変更禁止**の制約下での本質的な軽量化は「photos を genba 単位で集計する repository メソッド＋専用 provider（例: `yearPhotoCountsProvider(year)`）」の導入だが、DB 集計クエリと provider 境界の追加になるため今回は見送り、次増分の残課題とする（loading/error を 0 に潰さない方針は維持）。

検証（その2）: `dart format`/`dart analyze`（クリーン）・`flutter test` **1034件 緑**。追加テスト: `memory_edit_widgets_test.dart`（`_AddRow` の例外/Failure/成功の復帰）。**未実施（実機依存・成功扱いにしない）**: 画像ピッカーの実機での権限拒否・OS例外・選択中に画面破棄する競合はユニットテスト困難のためコード上のガードのみ（実機確認は未実施）。
