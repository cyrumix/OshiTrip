...
```powershell
# リポジトリ直下（OshiTrip/）で実行。既存 docs/.github/.gitignore は保持。
flutter create --org app.oshitrip --project-name oshi_trip `
  --platforms=android,ios --description "OshiTrip" .

# 生成後、Android applicationId/namespace と iOS bundle ID は
# 正式ID `app.oshitrip.mobile`（devは`.dev`、stagingは`.stg`）へ統一する。
